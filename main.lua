...
local ISFOCUSID = WarpZone.ISFOCUSID

WarpZone.badPickupsToForceCollide = {
    [PickupVariant.PICKUP_BIGCHEST]=true,
    [PickupVariant.PICKUP_COLLECTIBLE]=true,
    [PickupVariant.PICKUP_BROKEN_SHOVEL]=true,
}
local badPickupsToForceCollide = WarpZone.badPickupsToForceCollide

WarpZone.NIGHTMARE_TICK_DAMAGETABLE = {
    [0] = .5, .75, 1, 1.25, 1.5
}
local NIGHTMARE_TICK_DAMAGETABLE = WarpZone.NIGHTMARE_TICK_DAMAGETABLE
...
local function firePopTear(player, playYV, angleOffset)
    --local direction = player:GetAimDirection() * 15
    local direction = player:GetLastDirection() * 15
    ---@type EntityTear
    local tear = player:FireTear(player.Position, direction:Rotated(angleOffset or 0), false, false, true, nil, 1)
...
WarpZone.DoubleTapCallback = {
    {function(player, direction) --pop pop
        local count = player:GetCollectibleNum(WarpZone.WarpZoneTypes.COLLECTIBLE_POPPOP)
        if count > 0 then -- player:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_POPPOP) then
            local data = player:GetData()
            ...
            --if data.WarpZone_unsavedata.arrowTimeDelay <= 0 then
                ...
                local tearcount = math.min(5, count)
                for i=1, tearcount do
                    local angleOffset = (i-1)*10 - 5*(tearcount-1)
                    firePopTear(player, true, angleOffset)
                end
...
function WarpZone.ShotArrow(pos, vec, source, tearscale, dmg, sub, sync)
    ...
    tear.CollisionDamage = dmg or tear.CollisionDamage
    tear.Scale = tearscale or tear.Scale
    tear:ResetSpriteScale()
...
function WarpZone:UIOnRender(player, renderoffset)
    ...
    if player:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_BOW_AND_ARROW) then
        ...
        data.WarpZone_data.maxArrowNum = math.ceil(30 / (player.MaxFireDelay + 1)) * numCollectibles + 2
...
function WarpZone:spawnCleanAward(RNG, SpawnPosition)
    ...
    for j=0, numPlayers-1, 1 do
        ...
        if player:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_NIGHTMARE_TICK) then
            if REPENTOGON then
                wdata.roomsClearedSinceTake = wdata.roomsClearedSinceTake + 1
                local roomsToSuck = math.max(10 - (2 * player:GetCollectibleNum(WarpZone.WarpZoneTypes.COLLECTIBLE_NIGHTMARE_TICK)), 1)
                if wdata.roomsClearedSinceTake % roomsToSuck == 0 then

                    local history = player:GetHistory()
                    local config = Isaac.GetItemConfig()
                    --local pos_to_delete = 1
                    for pos, k in pairs(history:GetCollectiblesHistory())  do
                        local itemID = k:GetItemID()
                        local conf = config:GetCollectible(itemID)
                        if not k:IsTrinket() and not conf:HasTags(ItemConfig.TAG_QUEST) and conf.Type ~= ItemType.ITEM_ACTIVE
                        and itemID ~= WarpZone.WarpZoneTypes.COLLECTIBLE_NIGHTMARE_TICK then
                            --if history:RemoveHistoryItemByIndex(pos-1) then
                                player:RemoveCollectible(itemID)
                                local itemconf = Isaac.GetItemConfig():GetCollectible(itemID)

                                --data.WarpZone_data.itemsSucked = data.WarpZone_data.itemsSucked + 1
                                wdata.itemsSucked = wdata.itemsSucked or {}
                                wdata.itemsSucked[itemconf.Quality] = wdata.itemsSucked[itemconf.Quality] 
                                    and (wdata.itemsSucked[itemconf.Quality]+1) or 1
                                wdata.itemsSucked.Num = wdata.itemsSucked.Num and (wdata.itemsSucked.Num + 1) or 1
                                ...
                                lostitem:ReplaceSpritesheet(1, itemconf.GfxFileName)
                ...
                wdata.roomsClearedSinceTake = wdata.roomsClearedSinceTake + 1
                local roomsToSuck = math.max(10 - (2 * player:GetCollectibleNum(WarpZone.WarpZoneTypes.COLLECTIBLE_NIGHTMARE_TICK)), 1)
                local itemsTakenHere = wdata.itemsTaken
                if wdata.roomsClearedSinceTake % roomsToSuck == 0 then
                    local rng = player:GetCollectibleRNG(WarpZone.WarpZoneTypes.COLLECTIBLE_NIGHTMARE_TICK)
                    local shift = 0
                    for j, item_tag in ipairs(wdata.itemsTaken) do
                        if player:HasCollectible(item_tag) == false then
                            table.remove(wdata.itemsTaken, j-shift)
                            table.remove(wdata.poolsTaken, j-shift)
                            shift = shift + 1
                        end
                    end
                    
                    local pos_to_delete = rng:RandomInt(#itemsTakenHere) + 1
                    if wdata.itemsTaken[pos_to_delete] ~= nil then
                        local config = Isaac.GetItemConfig():GetCollectible(wdata.itemsTaken[pos_to_delete])
                        if (wdata.itemsTaken[pos_to_delete] == WarpZone.WarpZoneTypes.COLLECTIBLE_NIGHTMARE_TICK or (config.Tags & ItemConfig.TAG_QUEST == ItemConfig.TAG_QUEST)) and #itemsTakenHere > 1 then
                            pos_to_delete = (pos_to_delete % #itemsTakenHere) + 1
                        end
                        
                        config = Isaac.GetItemConfig():GetCollectible(wdata.itemsTaken[pos_to_delete])
                        if wdata.itemsTaken[pos_to_delete] ~= WarpZone.WarpZoneTypes.COLLECTIBLE_NIGHTMARE_TICK and (config.Tags & ItemConfig.TAG_QUEST ~= ItemConfig.TAG_QUEST) then
                            local item_del = table.remove(wdata.itemsTaken, pos_to_delete)
                            local itemconf = Isaac.GetItemConfig():GetCollectible(item_del)
                            table.remove(wdata.poolsTaken, pos_to_delete)
                            player:RemoveCollectible(item_del)
                            
                            --wdata.itemsSucked = wdata.itemsSucked + 1
                            wdata.itemsSucked = wdata.itemsSucked or {}
                            wdata.itemsSucked[itemconf.Quality] = wdata.itemsSucked[itemconf.Quality] 
                                and (wdata.itemsSucked[itemconf.Quality]+1) or 1
                            wdata.itemsSucked.Num = wdata.itemsSucked.Num and (wdata.itemsSucked.Num + 1) or 1
        ...
        if wdata.roomsSinceBreak and wdata.roomsSinceBreak > 0 then
            wdata.roomsSinceBreak = wdata.roomsSinceBreak - 1
            if wdata.roomsSinceBreak == 0 then
...
function WarpZone:usePastkiller3x(collectible, rng, entityplayer, useflags, activeslot, customvardata)
    ...
    if entityplayer:HasCollectible(CollectibleType.COLLECTIBLE_BOOK_OF_VIRTUES) or useflags & UseFlag.USE_ALLOWWISPSPAWN ~= 0 then
        entityplayer:AddWisp(WarpZone.WarpZoneTypes.COLLECTIBLE_PASTKILLER, entityplayer.Position)
    end
    return {
        Discharge = false,
        Remove = activeslot == -1,
...
function WarpZone:UseFocus(collectible, rng, player, useflags, activeslot, customvardata)
    ...
    player:GetData().WarpZone_unsavedata.UsingFocus = {1, activeslot, useflags}
...
function WarpZone:OnPickupCollide(entity, Collider, Low)
    ...
    if entity.Type == EntityType.ENTITY_PICKUP and entity.Variant == tokenVariant then
        local data = data.WarpZone_data
        if not entity:GetData().Collected and data.maxArrowNum and data.numArrows < data.maxArrowNum then
            entity:GetData().Collected = true
            entity:GetSprite():Play("Collect")
            entity.Velocity = Vector(0,0)
            data.numArrows = data.numArrows + 1
    ...
    if entity.Type == EntityType.ENTITY_PICKUP and (entity.Variant == PickupVariant.PICKUP_COLLECTIBLE) and entity:ToPickup():GetData().Logged ~= true then
        ...
        if entity.SubType == WarpZone.WarpZoneTypes.COLLECTIBLE_BOW_AND_ARROW then
            data.WarpZone_data.numArrows = math.min(data.WarpZone_data.maxArrowNum or 6, data.WarpZone_data.numArrows + 6)
        end
    end
    return nil
end
    ...
    if pickup.Variant == tokenVariant then
        ...
        for i=0, game:GetNumPlayers()-1 do
            ...
            if player:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_BOW_AND_ARROW) then
                ...
                if data.numArrows < data.maxArrowNum and ppos:Distance(entpos) < 80 then
                    pickup.Velocity = (ppos-entpos):Resized(16)
                    near = true
                end
            end
        end
        if near then
            data.hasgridcoll = data.hasgridcoll or pickup.GridCollisionClass/1
            pickup.GridCollisionClass = EntityGridCollisionClass.GRIDCOLL_NONE
        else
            if data.hasgridcoll then
                pickup.GridCollisionClass = data.hasgridcoll
                data.hasgridcoll = nil
            end
        end
    end
...
function WarpZone:EvaluateCache(entityplayer, Cache)
    ...
    if Cache == CacheFlag.CACHE_DAMAGE then
        local D8Edenmulti = 1
        if REPENTOGON then
            D8Edenmulti = entityplayer:GetD8DamageModifier() + entityplayer:GetEdenDamage()
        end
        ...
        --if entityplayer:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_NIGHTMARE_TICK) then
            ...
            local suctab = data.WarpZone_data.itemsSucked
            local socdmg = 0
            if suctab then
                for i=0, 4 do
                    socdmg = socdmg + (suctab[i] or 0) * NIGHTMARE_TICK_DAMAGETABLE[i]
                end
            end
            damageToAdd = damageToAdd + socdmg
        --end
    ...
    if Cache == CacheFlag.CACHE_TEARFLAG then
        ...
        if suctab then
            ...
            if suctab.Num > 3 then
                flags = flags | TearFlags.TEAR_FEAR
            end
            entityplayer.TearFlags = entityplayer.TearFlags | flags
        end
    end
...
local IsLost = {[PlayerType.PLAYER_THELOST]=true, [PlayerType.PLAYER_THELOST_B]=true}
...
function WarpZone:postPlayerUpdate(player)
    ...
    if player:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_POPPOP) then
        local tearcount = math.min(5, player:GetCollectibleNum(WarpZone.WarpZoneTypes.COLLECTIBLE_POPPOP))
        if data.WarpZone_data.arrowTimeThreeFrames == 1 then
            for i=1, tearcount do
                local angleOffset = (i-1)*10 - 5*(tearcount-1)
                firePopTear(player, false, angleOffset)
            end
        end
    ...
    if unsave.UsingFocus then
        ...
        if Input.IsActionPressed(act, player.ControllerIndex) then
            local canRS = not IsLost[player:GetPlayerType()] and (player:CanPickRedHearts() or player:CanPickSoulHearts())
            ...
            if unsave.UsingFocus[1] >= 120 and canRS then
                ...
                or unsave.UsingFocus[3] and unsave.UsingFocus[3] & UseFlag.USE_ALLOWWISPSPAWN ~= 0 then
                    for i=1,3 do
                        player:AddWisp(WarpZone.WarpZoneTypes.COLLECTIBLE_FOCUS, player.Position, true)
                    end
                end
...
function WarpZone:checkTear(entitytear)
    ...
    if player and player:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_NIGHTMARE_TICK) then
...
function WarpZone:updateLaser(laser)
    ...
    if laser.Visible and data.WZ_ShotArrow ~= nil and player then
        if data.WZ_ShotArrow == true then
            ...
            data.WZ_ShotArrow = 30
        elseif data.WZ_ShotArrow then --== false and player:GetData().WarpZone_data.numArrows > 0 then
            data.WZ_ShotArrow = data.WZ_ShotArrow - 1
            ...
            if data.WZ_ShotArrow <= 0 then
                if player:GetData().WarpZone_data.numArrows > 0 then
                    data.WZ_ShotArrow = 30
                else
                    data.WZ_ShotArrow = nil
                end
            end
        end
    end
...
function WarpZone:updateTear(entitytear)
    ...
    if data then
        ...
        if (not data.WarpZone_data or not data.WarpZone_data.IsBow) and player and tear:HasTearFlags(TearFlags.TEAR_LUDOVICO) then
            if player:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_BOW_AND_ARROW) then
                ...
                local wdata = data.WarpZone_data
                wdata.IsBow = true
                wdata.BowShotDelay = player.MaxFireDelay*4
                wdata.BowMaxDelay = wdata.BowShotDelay
                wdata.BowArrowPiercing = nil
        ...
        if data.WarpZone_data then
            local wdata = data.WarpZone_data
            if  wdata.BowArrowPiercing == 2 or data.WarpZone_data.BowArrowPiercing == 3 then
                spr:Load("gfx/arrow tear.anm2", true)
                spr:Play(spr:GetDefaultAnimation())
                spr:Play("RegularTear6")

                if wdata.BowArrowPiercing == 2 then
                    wdata.BowArrowPiercing = 1
                    if player then
                        local pda = player:GetData().WarpZone_data
                        pda.numArrows = pda.numArrows - 1
                    end
                elseif wdata.BowArrowPiercing == 3 then
                    wdata.BowArrowPiercing = -1
                end
                tear:AddTearFlags(TearFlags.TEAR_PIERCING)
                ...
                if wdata.trail then
                    wdata.trail.ParentOffset = tear.PositionOffset
                end
                tear:ResetSpriteScale()
            ...
            elseif wdata.BowArrowPiercing == 1 or wdata.BowArrowPiercing == -1 then
            ...
            if player and wdata.IsBow then
                local data = wdata
                    ...
                    if player:GetData().WarpZone_data.numArrows > 0 then
                        ...
                        if data.BowShotDelay < 0 then
                            ...
                            for i=1, loop do
                                ...
                                local tera = WarpZone.ShotArrow(tear.Position, tarvel:Resized(player.ShotSpeed*15), player, nil, nil, nil, true )
...
function WarpZone:OnFrame(player)
    local data = player:GetData()
        ...
        if data.reticle ~= nil then --player:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_IS_YOU) and
...
function WarpZone:OnKnifeUpdate(knife)
    ...
    if player and knife.Variant == 0 then
        if knife:IsFlying() then
            local pdata = player:GetData().WarpZone_data
            if player:HasCollectible(WarpZone.WarpZoneTypes.COLLECTIBLE_BOW_AND_ARROW) then
                if not data.WZ_ShotArrow and pdata.numArrows > 0 then
                    data.WZ_ShotArrow = 30
                    
                    local params = player:GetTearHitParams(WeaponType.WEAPON_TEARS, 1)
                    WarpZone.ShotArrow(knife.Position, Vector.FromAngle(knife.Rotation):Resized(player.ShotSpeed*12), 
                        player, params.TearScale, player.Damage*.5, nil, true)
                end

                if data.WZ_ShotArrow then --== false and player:GetData().WarpZone_data.numArrows > 0 then
                    data.WZ_ShotArrow = data.WZ_ShotArrow - 1

                    if  data.WZ_ShotArrow % 3 == 0 then --laser.FrameCount < 30 and
                        local params = player:GetTearHitParams(WeaponType.WEAPON_TEARS, .7)
                        local miss = knife:GetDropRNG():RandomInt(30)-15
                        WarpZone.ShotArrow(player.Position, Vector.FromAngle(knife.Rotation+miss):Resized(player.ShotSpeed*12), 
                            player, params.TearScale, player.Damage*.25, true)
                    end
                    if data.WZ_ShotArrow <= 0 then   --laser.FrameCount % 30 == 0 then
                        local miss = knife:GetDropRNG():RandomInt(30)-15
                        local params = player:GetTearHitParams(WeaponType.WEAPON_TEARS, 1)
...
local function update_cache(_, player, cache_flag)
	if cache_flag == CacheFlag.CACHE_FAMILIARS then
        ...
        if Isaac.GetChallenge() == WarpZone.WarpZoneTypes.CHALLENGE_BLIND_JOHNNY then
            john_pickups = math.max(1, john_pickups)
        end
...
function WarpZone:fireGlove(player, showeff, hitlist)
    ...
    for i=1 , #list do
        ...
        if not hitlist[ent.Index] and ent:IsActiveEnemy() then
            ...
            if REPENTOGON then
                ent:AddKnockback(ref, aim:Resized(30), 20, true)
            else
    ...
    if REPENTOGON then
        ...
        local list = Isaac.FindInRadius(attackpos + (aim * 20), 50, EntityPartition.PICKUP)
        for i=1 , #list do
            local entT = list[i]
            if entT.Type == 5 and not hitlist[entT.Index] and not badPickupsToForceCollide[entT.Variant] then
                hitlist[entT.Index] = true
                if player:ForceCollide(entT, true) then

                else
                    entT:AddVelocity(aim:Resized(10 / entT.Mass * 10))
                end
            end
        end
    end
