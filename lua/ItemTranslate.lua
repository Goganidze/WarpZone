...
local CollectibleType = CollectibleType
...
--if not ItemTranslate or ItemTranslate.Ver and ItemTranslate.Ver < ITver then
			...
			for i, k in pairs(PocketItemStrings) do
				...
				if k.Name and k.player and k.player:ToPlayer() then
					...
					if pType == PlayerType.PLAYER_ESAU then
						...
						font:DrawStringScaledUTF8(str, Pos.X, Pos.Y + 5+SizeOffset, fontSize, fontSize, KColor(ah, ah, ah, ah), 1, false)
					else
						if id == 0 then
							if pType == PlayerType.PLAYER_JACOB then
								...
								local Pos = Corner + Offset + shakeOffset
								font:DrawStringScaledUTF8(str, Pos.X, Pos.Y + 13+SizeOffset, fontSize, fontSize, KColor(ah, ah, ah, ah), 0,
									true)
							else
								...
								font:DrawStringScaledUTF8(str, Pos.X, Pos.Y + 13+SizeOffset, fontSize, fontSize, KColor(ah, ah, ah, ah), 1,
									false)
							end
						elseif id == 1 then
							...
							font:DrawStringScaledUTF8(str, Pos.X, Pos.Y + 5 + heartoffset, 0.5, 0.5, KColor(ah, ah, ah,
								ah), 0, true)
						elseif id == 2 then
							...
							font:DrawStringScaledUTF8(str, Pos.X + 1, Pos.Y + 5 + heartoffset, 0.5, 0.5,
								KColor(ah, ah, ah, ah), 0, true)
						elseif id == 3 then
							local ActiveItemOffset = k.IsActiveItem and Vector(-2.5, 0) or Vector.Zero
	...
	local HUDRENDERCALLBACKID = ModCallbacks.MC_POST_RENDER
	if Renderer then
	...
	ITrsl:AddCallback(HUDRENDERCALLBACKID, ITrsl.RenderPocketItemName)
